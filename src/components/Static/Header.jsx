                    ...
                    {Items.map((i, _) => (
                        <Link href={i?.redirect} passHref key={_}>
                            <div className="relative flex flex-col h-full items-center">
                                <p className={`cursor-pointer transition-all duration-200 ${router.asPath === i.redirect ? 'text-cIndigo font-medium' : 'text-black/50 hover:text-black/100 font-normal'}`}>{i.label}</p>
